...
const TABS = [
    { id: 'students', label: 'Students', color: 'blue' },
    { id: 'groups', label: 'Elective Groups', color: 'violet' },
    { id: 'allocation', label: 'Allocation', color: 'emerald' }
];
    ...
    async function performUpload(fileArg) {
        ...
        try {
            ...
            if (uploadedCount > 0) {
                ...
                const serverMsg = res.data?.error || 'No valid rows found in the file. Nothing was uploaded.';
                setUploadResult({ success: false, message: serverMsg });
...
function AllocationTab({ instanceId }) {
    ...
    // ── KEY FIX: Derive whether allocation has already been done ──
    const isAlreadyAllocated = useMemo(() => {
        return !!lastAllocatedMethod || rows.length > 0;
    }, [lastAllocatedMethod, rows]);
    ...
    // ── KEY FIX: Guard against re-allocation without undo ──
    async function handleAllocate() {
        if (resolvedInstanceId == null) return;
        if (isAlreadyAllocated) {
            const msg = 'Allocation has already been completed for this instance. Please "Undo Allocations" first if you want to re-run.';
            setError(msg);
    ...
    return (
        <>
            ...
            {/* ── KEY FIX: Show clear status banner when allocation is already done ── */}
            {isAlreadyAllocated ? (
                <div className="mb-4 rounded-xl border border-indigo-200 bg-indigo-50 px-4 py-3 text-sm font-medium text-indigo-900">
                    Allocation has already been completed using: <strong>{getAllocationMethodLabel(lastAllocatedMethod || 'preference_cgpa')}</strong>.
                    If you want to re-run with a different method, click <strong>"Undo Allocations"</strong> first.
                </div>
            ) : null}

            {error ? <div className="mb-4 rounded border border-red-200 bg-red-50 p-3 text-sm text-red-700">{error}</div> : null}

            {/* ── Disable method selection when already allocated ── */}
                    ...
                    {ALLOCATION_METHOD_OPTIONS.map((option) => (
                        <label
                            ...
                            className={`flex cursor-pointer items-start gap-3 rounded-xl border p-4 transition ${
                                !isAlreadyAllocated && allocationMethod === option.value
                        ...
                        >
                            <input
                                ...
                                disabled={allocating || loading || isAlreadyAllocated}
                            />
                            <span>
            ...
            <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-end">
                <div className="flex flex-wrap items-center gap-3">
                    {/* ── KEY FIX: Disable Run Allocation when already done ── */}
                    <button
                        type="button"
                        onClick={handleAllocate}
                        disabled={allocating || loading || isAlreadyAllocated}
                        className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-60"
                        title={isAlreadyAllocated ? 'Undo allocations first to re-run' : 'Run Allocation'}
                    >
                        {allocating ? 'Allocating...' : isAlreadyAllocated ? 'Already Allocated' : 'Run Allocation'}
                    </button>
                    <button
                        type="button"
            ...
            <div className="overflow-hidden rounded-xl bg-white shadow-md">
                <div className="border-b border-gray-200">
                    <nav className="flex overflow-x-auto">
                        {TABS.map((tab) => {
                            const colors = TAB_COLOR_MAP[tab.color] || TAB_COLOR_MAP.blue;
                            ...
                            return (
                                <button
                                    ...
                                    className={`whitespace-nowrap border-b-2 px-6 py-3.5 text-lg font-bold transition-colors ${
                                        isActive
                                            ? `${colors.activeBorder} ${colors.activeText} ${colors.activeBg}`
                                            : `border-transparent text-gray-500 ${colors.hoverText} hover:border-gray-300`
                                    }`}
                                >
                                    {tab.label}
                                </button>
